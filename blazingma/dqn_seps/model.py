...
from blazingma.utils.models import MultiAgentSEPSNetwork
...
class QNetwork(nn.Module):
    ...
    def act(self, inputs, seps_indices, epsilon):
        if epsilon > random.random():
            actions = self.action_space.sample()
            return actions
        with torch.no_grad():
            inputs = [torch.from_numpy(i).to(self.device) for i in inputs]
            actions = [x.argmax(dim=0).cpu().item() for x in self.critic(inputs, seps_indices)]

        return actions

    def update(self, batch, seps_indices):
        ...
        with torch.no_grad():
            q_tp1_values = self.critic(nobs, seps_indices)
            q_next_states = self.target(nobs, seps_indices)
        all_q_states = self.critic(obs, seps_indices)
