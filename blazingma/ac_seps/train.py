...
def main(envs, logger, **cfg):
    ...
    for step in tqdm(range(1, cfg.total_steps + 1)):
        ...
        with torch.no_grad():
            next_value = model.get_target_value(split_obs(batch_obs[cfg.n_steps, :, :]), seps_indices)
        ...
        values, action_log_probs, entropy = model.evaluate_actions(split_obs(batch_obs[:-1]), split_act(batch_act), seps_indices)
